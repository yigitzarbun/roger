...
const TrainerFavouritesResults = () => {
    ...
    <div className={styles["trainer-favourites-container"]}>
      ...
      {myFavourites?.length > 0 ? (
        <table>
          ...
          <tbody>
            {myFavourites
              ?.slice(myFavourites?.length - 1)
              ?.map((myFavourite) => (
                <tr key={myFavourite.favourite_id}>
                  <td>
                    <AiFillStar
                      className={styles["remove-fav-icon"]}
                      onClick={() =>
                        handleRemovaFavourite(myFavourite.favourite_id)
                      }
                    />
                  </td>
                  <td>
                    <Link
                      to={`${paths.EXPLORE_PROFILE}${
                        users?.find(
                          (user) => user.user_id === myFavourite.favouritee_id
                        )?.user_type_id
                      }/${myFavourite.favouritee_id}`}
                    >
                      <img
                        src={
                          users?.find(
                          ...
                          players?.find(
                            (player) =>
                              player.user_id === myFavourite.favouritee_id
                          )?.image
                            ? players?.find(
                                (player) =>
                                  player.user_id === myFavourite.favouritee_id
                            ...
                            : users?.find(
                                (user) =>
                                  user.user_id === myFavourite.favouritee_id
                              )?.user_type_id === 2 &&
                              trainers?.find(
                                (trainer) =>
                                  trainer.user_id === myFavourite.favouritee_id
                              )?.image
                            ? trainers?.find(
                                (trainer) =>
                                  trainer.user_id === myFavourite.favouritee_id
                            ...
                            : users?.find(
                              ...
                              clubs?.find(
                                (club) =>
                                  club.user_id === myFavourite.favouritee_id
                              )?.image
                            ? clubs?.find(
                                (club) =>
                                  club.user_id === myFavourite.favouritee_id
                  ...
                  <td>
                    ...
                    >
                      {users?.find(
                        ...
                        ? clubs?.find(
                            (club) => club.user_id === myFavourite.favouritee_id
                          )?.club_name
                        : ""}
                    </Link>
                  </td>
...
export default TrainerFavouritesResults;
