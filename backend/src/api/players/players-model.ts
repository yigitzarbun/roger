    ...
    try {
      const playerDetails = await db
        .select(
          ...
          db.raw(
            "COALESCE(COUNT(CASE WHEN match_scores.match_score_status_type_id = 3 THEN 1 END), 0) as totalMatches"
          ),
          db.raw(
          ...
          db.raw(
            "COALESCE(SUM(CASE WHEN match_scores.winner_id != players.user_id AND (match_scores.match_score_status_type_id = 3) THEN 1 ELSE 0 END), 0) as lostMatches"
          ),
          db.raw(
